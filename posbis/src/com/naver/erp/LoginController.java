...
public class LoginController {
	...
	public ModelAndView myPageForm() {		// 메소드 이름은 상관 없음.
		 ...
		 * try {
		 * 
		 * mypageDTO mypageList=this.loginService.getmypageList();
...
//	
	...
	//-----------------------------------------------------------------------------
		...
		//구 체크 후 동체크
		@RequestMapping(
				value="/addrDongProc.do"
				, method=RequestMethod.POST
				, produces="application/json;carset=UTF-8"
		)
		@ResponseBody
		public List<String> addrDongProc(
				@RequestParam(value="addr_gu") String addr_gu
		) {
			 
 
		     // webContent/WEB-INF/spring/appServlet 폴더 안의 servlet-context.xml 파일 안에 46~49 줄이 접두사 , 접미사 설정이 되어있음. 
		...
		//  업종1 검색시 업종2 구하기
		@RequestMapping(
				value="/businessTypeName2Proc.do"
				, method=RequestMethod.POST
				, produces="application/json;carset=UTF-8"
		)
		@ResponseBody
		public List<String> businessTypeName2Proc(
		...
		//  회원가입 등록 ajax 
		@RequestMapping(
				value="/joinRegProc.do"
				, method=RequestMethod.POST	
				, produces="application/json;carset=UTF-8"
		)
		 @ResponseBody
		 public int joinRegProc(
				 joinDTO joinDTO
		) {
			//사용자 등록 
			int insertJoinCnt = 0;
...
//-----------------------------------------------------------------------------		
	//사업자 아이디 중복
		...
		@RequestMapping(
				value="/chekckBusinessNoProc.do"
				, method=RequestMethod.POST
				, produces="application/json;carset=UTF-8"
		)
		@ResponseBody
		public int chekckBusinessNoProc( 
